...
if (isset($_SESSION["student_email"])) {
    ...
    $sql = "SELECT * FROM favourite WHERE Tutorial_ID = '$Tutorial_ID' AND Email = '$email'";
    $run_query = mysqli_query($con,$sql);
    $count = mysqli_num_rows($run_query);
    if($count > 0){
        echo "<script>
        alert('Favourite is already exist.');
    ...
    echo"<script>
        alert('Only student can add favourite.');
        javascript:history.go(-1);
        </script>";
};
?>	